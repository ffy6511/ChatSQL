...
class ERDiagramStorage {
  ...
  // 计算图表统计信息
  private calculateStats(data: ERDiagramData): { entityCount: number; relationshipCount: number } {
    return {
  ...
  async listDiagrams(): Promise<ERDiagramMetadata[]> {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        const diagrams = request.result.map((item: StoredERDiagram) => item.metadata);
        // 按更新时间倒序排列
