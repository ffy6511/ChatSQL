...
const LLMResultView: React.FC<LLMResultViewProps> = ({ outputs }) => {
  return (
    <div className={`${styles.resultCard} ${styles.tableStyles}`}>
      {/* 描述 */}
      <Typography.Title level={4} style={{ color: "var(--primary-text)" }}>
        题目描述
      </Typography.Title>
      <Typography.Paragraph style={{ color: "var(--secondary-text)" }}>
        {outputs.description}
      ...
      {/* 标签 */}
      <Typography.Title level={5} style={{ color: "var(--primary-text)" }}>
        标签
      </Typography.Title>
      ...
      {outputs.tuples && (
        <>
          <Title level={5} style={{ color: "var(--primary-text)" }}>
            样例数据
          </Title>
      ...
      {outputs.hint && (
        <>
          ...
          <Paragraph style={{ color: "var(--secondary-text)" }}>
            {outputs.hint}
          </Paragraph>
        </>
      )}
