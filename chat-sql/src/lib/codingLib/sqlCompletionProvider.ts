...
export const SQL_KEYWORDS = [
  ...
  },
  {
    label: "LIKE",
    documentation:
      'Pattern matching. % represents any number of characters, _ represents a single character.  \nSyntax: WHERE column LIKE pattern;  \nExample: SELECT * FROM products WHERE name LIKE "%phone%";',
  },
  {
    label: "IS NULL",
    documentation:
...
export function createSQLCompletionProvider(
  monaco: any,
  tableStructures: TableStructure[] | undefined,
) {
  ...
  return {
    ...
    provideCompletionItems: (model: any, position: any) => {
      ...
      const isAfterJoin =
        /\b(JOIN|INNER\s+JOIN|LEFT\s+JOIN|RIGHT\s+JOIN|NATURAL\s+JOIN)\s+$/i.test(
          textBefore,
        );
      ...
      if (isSpaceTrigger) {
        ...
        // 检查是否在表名后面（不带点号）
        // 匹配表名后面跟着空格的情况
        const isAfterTableName =
          tableStructures?.some((table) => {
      ...
      if (currentTableName) {
        // 查找匹配的表
        const table = tableStructures?.find(
          (t) => t.tableName.toUpperCase() === currentTableName.toUpperCase(),
        );
      ...
      if (isAfterSelect) {
        ...
        if (tableStructures) {
          for (const table of tableStructures) {
            for (const column of table.columns) {
              // 添加带表名的列
              const qualifiedColumnId = createSuggestionId(
                "qualified_column",
      ...
      if (tableStructures && tableStructures.length > 0) {
        ...
        for (const table of tableStructures) {
          ...
          for (const column of table.columns) {
            ...
            if (
              !addedLabels.has(
                `QUALIFIED_COLUMN:${table.tableName.toUpperCase()}.${column.name.toUpperCase()}`,
              )
            ) {
              const qualifiedColumnId = createSuggestionId(
                "qualified_column",
