...
const SQLEditor: React.FC<SQLEditorProps> = ({
  ...
  const handleEditorWillMount = useCallback(
    (monaco: any) => {
      ...
      } = require("@/lib/codingLib/sqlCompletionProvider");
      const {
        createSQLHoverProvider,
      } = require("@/lib/codingLib/sqlHoverProvider");

      // 尝试清除所有已注册的SQL自动补全提供器
      try {
        // 如果有之前注册的提供器，先取消注册
        if (completionProviderRef.current) {
          completionProviderRef.current.forEach((provider: any) =>
            provider.dispose(),
          );
        ...
        if (
          monaco.languages._builtinProviders &&
          monaco.languages._builtinProviders["sql"]
        ) {
