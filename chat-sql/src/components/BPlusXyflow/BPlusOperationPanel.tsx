...
const validateInput = (value: string): boolean => {
  ...
  return parts.every((part) => {
    ...
    return (
      !isNaN(num) && Number.isInteger(num) && num >= -99999 && num <= 99999
    );
  });
};
...
const BPlusOperationPanel: React.FC<BPlusOperationPanelProps> = ({
  ...
  const handleInsert = useCallback(async () => {
    if (!validateInput(insertValue)) {
      showMessage(
        "插入失败：请输入一个或多个空格分隔的有效整数（-99999~99999）",
        "warning",
      );
      return;
    }
    ...
    try {
      ...
      if (successCount === valuesToInsert.length) {
        ...
        showMessage(
          `部分成功：插入 ${successCount} 个key，有 ${valuesToInsert.length - successCount} 个key失败（可能已经存在）`,
          "warning",
        );
      } else {
  ...
  const handleDelete = useCallback(async () => {
    if (!validateInput(deleteValue)) {
      showMessage(
        "删除失败：一个或者多个空格分隔的有效整数（-99999~99999）",
        "warning",
      );
      return;
    }
    ...
    try {
      ...
      if (successCount === valuesToDelete.length) {
        ...
        showMessage(
          `部分成功：删除 ${successCount} 个key，有 ${valuesToDelete.length - successCount} 个key失败（可能不存在）`,
          "warning",
        );
      } else {
      ...
      <Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
        ...
        <Box sx={{ display: "flex", gap: 2, alignItems: "center" }}>
          <TextField
            ...
            helperText={
              insertValue !== "" && !validateInput(insertValue)
                ? "请输入有效的整数（-99999～99999）"
                : ""
            }
            sx={{
              flex: 1,
        ...
        <Box sx={{ display: "flex", gap: 2, alignItems: "center" }}>
          <TextField
            ...
            helperText={
              deleteValue !== "" && !validateInput(deleteValue)
                ? "请输入有效的整数（支持正负数和0）"
                : ""
            }
            sx={{
