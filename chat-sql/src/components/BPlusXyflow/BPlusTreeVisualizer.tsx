...
import {
  AnimationManager,
  AnimationState,
} from "../../lib/bplusLib/animationManager";
...
const layoutNodes = (
  ...
  levels.forEach((level, levelIndex) => {
    ...
    nodesInLevel.sort((a, b) => {
      const firstKeyA =
        (a.data.keys.find((k) => k !== null) as number | undefined) ?? Infinity;
      const firstKeyB =
        (b.data.keys.find((k) => k !== null) as number | undefined) ?? Infinity;
...
const convertBPlusTreeToFlowData = (
  algorithm: BPlusTreeAlgorithm,
  order: number,
  ...
  allNodes.forEach((node) => {
    ...
    const paddedKeys = [
      ...nodeKeys,
      ...Array(Math.max(0, order - 1 - nodeKeys.length)).fill(null),
    ];
  ...
  return (
    <Panel position="bottom-right" className="custom-zoom-controls">
      ...
      <Tooltip title="重置" placement="top">
        <button onClick={onReset} className="zoom-button">
          ↻
        </button>
      </Tooltip>
    </Panel>
...
const BPlusTreeVisualizerInner: React.FC<BPlusTreeVisualizerProps> = ({
  ...
  const [animationState, setAnimationState] = useState<AnimationState>(
    animationManagerRef.current.getState(),
  );
  ...
  const commandExecutorRef = useRef(
    new CommandExecutor({
  ...
  const runOperation = useCallback(
    async (operation: "insert" | "delete", key: number) => {
      const algorithm = bPlusTreeAlgorithmRef.current;
      const manager = animationManagerRef.current;
      ...
      const commands =
        operation === "insert"
          ? algorithm.insertElement(key)
          : algorithm.deleteElement(key);
  ...
  const handleInsert = useCallback(
    async (key: number): Promise<boolean> => {
    ...
    <Box sx={{ height: "100%", width: "100%", position: "relative" }}>
      ...
      >
        <AnimationControls
          ...
          onSpeedChange={(speed) =>
            setSettings((s) => ({ ...s, animationSpeed: speed }))
          }
