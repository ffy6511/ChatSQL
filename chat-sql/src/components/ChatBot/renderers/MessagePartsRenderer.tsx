...
const ActionMenu: React.FC<{
  ...
  const handleVisualize = () => {
    // 根据内容类型确定可视化类型
    const visualizeType = isERDiagram
      ? "er-diagram"
      : contentType === "sql"
        ? "sql"
        : "er-diagram";
  ...
  const renderPart = (part: AgentOutputPart, index: number) => {
    ...
    switch (type) {
      case "text":
        return (
          <Box key={index} sx={{ mb: 2 }}>
            <MarkdownRenderer message={mockMessage} isUser={isUser} />
          </Box>
        );
      ...
      case "sql":
        return (
          <Box key={index} sx={{ mb: 2 }}>
            <SqlRenderer message={mockMessage} isUser={isUser} />
          </Box>
        );
      ...
      case "json":
        return (
          <Box key={index} sx={{ mb: 2 }}>
            <JsonRenderer message={mockMessage} isUser={isUser} />
          </Box>
        );
