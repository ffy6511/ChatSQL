...
import { Box, ButtonGroup, Button, Tooltip, Typography } from "@mui/material";
import {
  Chat as ChatIcon,
    ...
    <Box sx={{ p: 2, borderBottom: "1px solid var(--card-border)" }}>
      ...
      >
        {Object.values(AGENTS_INFO).map((agent) => {
          const IconComponent =
            ICON_MAP[agent.icon as keyof typeof ICON_MAP] || ChatIcon;
          ...
          return (
            <Tooltip key={agent.type} title={agent.description} placement="top">
              <Button
                ...
                startIcon={
                  <IconComponent sx={{ fontSize: "16px !important" }} />
                }
                sx={{
                  minWidth: "auto",
                  flex: 1,
