...
import { ERRelationship, ERConnection, ERAttribute } from '@/types/ERDiagramTypes/erDiagram';
import {
  dataTypeParamConfig,
  dataTypeOptions,
  parseDataType,
...
const RelationshipListView: React.FC = () => {
  const { state, deleteRelationship, setSelectedElement, updateConnection, 
    addRelationshipAttribute, deleteRelationshipAttribute, updateRelationshipAttribute } = useERDiagramContext();
  ...
  const handleDeleteRelationshipAttributeFromMenu = async (relationshipId: string, attributeId: string) => {
  ...
  const handleAttributeNameSave = async (relationshipId: string, attributeId: string) => {
    ...
    if (newName !== undefined) {
      ...
      await updateRelationshipAttribute(relationshipId, attributeId, { name: finalName });
  ...
  const handleAttributeTypeChange = (relationshipId: string, attributeId: string, dataType: string) => {
    // 切换类型时，初始化参数输入
  ...
  // 处理关系属性参数变化
  const handleRelationshipParamChange = async (relationshipId: string, attributeId: string, paramIndex: number, value: string, typeName: string) => {
    // 获取当前属性的参数
    const relationship = relationships.find(r => r.id === relationshipId);
    const attribute = relationship?.attributes?.find(attr => attr.id === attributeId);
    if (!attribute) return;
    ...
    // 保存更新
    await updateRelationshipAttribute(relationshipId, attributeId, { dataType: newDataType });
  };
  ...
  // 渲染关系属性项（支持内联编辑）
  const renderRelationshipAttributeItem = (attribute: ERAttribute, relationshipId: string) => {
    // 解析当前类型和参数
    ...
    return (
      ...
      >
        <Stack direction="row" alignItems="center" spacing={1} sx = {{ overflow: 'hidden', flexWrap: 'nowrap'}}>
          <Box sx = {{display: 'flex', gap: 1, flexGrow: 1}}>
            <TextField
              ...
              onBlur={() => handleAttributeNameSave(relationshipId, attribute.id)}
              onKeyDown={(e) => {
            ...
            <Autocomplete
              ...
              onChange={(_, newValue) => {
                if (newValue) {
                  handleAttributeTypeChange(relationshipId, attribute.id, newValue);
                }
              }}
        ...
        >
                ...
                return (
                  <MenuItem key={label} sx={{ flexDirection: 'column', alignItems: 'stretch', py: 1 }}>
                    <TextField
                      ...
                      size="small"
                      value={currentValue}
                      onChange={(e) => handleRelationshipParamChange(relationshipId, attribute.id, idx, e.target.value, typeName)}
          ...
          <MenuItem
            onClick={() => handleDeleteRelationshipAttributeFromMenu(relationshipId, attribute.id)}
            sx={{ color: 'error.main' }}
          >
                ...
                <CardContent>
                  ...
                  <Stack spacing={0.5}>
                    ...
                    {relationship.attributes && relationship.attributes.length > 0 ? (
                      relationship.attributes.map(attr => renderRelationshipAttributeItem(attr, relationship.id))
                    ) : (
