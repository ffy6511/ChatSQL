...
import HistoryModal, {
  HistoryRecord,
} from "@/components/ERDiagram/core/QuizHistoryModal";
...
const ERAssistantPanel: React.FC<ERAssistantPanelProps> = () => {
  ...
  const convertChatMessagesToMessages = useCallback(
  ...
  const convertToHistoryRecords = useCallback(
    (sessions: ChatSession[]): HistoryRecord[] => {
      // 合并聊天会话和ER图特定记录
  ...
  const currentAgentType =
    ER_ASSISTANT_TABS[activeTabIndex]?.agentType || AgentType.ER_QUIZ_GENERATOR;
