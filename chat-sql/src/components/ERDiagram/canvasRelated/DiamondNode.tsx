...
const DiamondNode: React.FC<NodeProps> = ({ data, selected, id }) => {
  const { label, description, attributes, isWeakRelationship } =
    data as DiamondNodeData;
  ...
  const hasTooltipContent =
    description || (attributes && attributes.length > 0);

  const nodeContent = (
    <div
      className={`${styles.diamondNode} ${selected ? styles.selected : ""} ${
        isWeakRelationship ? styles.weakRelationship : ""
      }`}
      onClick={handleClick}
