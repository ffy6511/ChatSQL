...
import BPlusTreeVisualizer, {
  TreeState,
  BPlusTreeOperations,
} from "@/components/BPlusXyflow/BPlusTreeVisualizer";
...
import NewSessionModal, {
  NewSessionFormData,
} from "@/components/BPlusHistory/NewSessionModal";
...
import {
  getBPlusHistoryStorage,
  BPlusHistoryStorage,
} from "@/lib/bplusLib/historyStorage";
...
const BPlusHistoryPage: React.FC = () => {
  ...
  const [currentSession, setCurrentSession] = useState<HistorySession | null>(
    null,
  );
  ...
  // 清理确认对话框状态
  const [isClearAllDialogOpen, setIsClearAllDialogOpen] =
    useState<boolean>(false);
  const [isClearingAll, setIsClearingAll] = useState<boolean>(false);

  // B+树操作接口
  const [treeOperations, setTreeOperations] =
    useState<BPlusTreeOperations | null>(null);

  // 历史存储服务
  const [historyStorage, setHistoryStorage] =
    useState<BPlusHistoryStorage | null>(null);
  ...
  const handleTreeStateChange = useCallback(
    async (state: TreeState) => {
      ...
      try {
        ...
        // 2. 获取更新后的会话并更新当前会话状态
        const updatedSession = await historyStorage.getSession(
          currentSession.id,
        );
        if (updatedSession) {
  ...
  const handleStepSelect = useCallback(
    (stepIndex: number) => {
      ...
      if (currentSession && currentSession.steps[stepIndex]) {
        ...
        showMessage(
          `已回溯到步骤: ${stepIndex + 1} - ${selectedStep.description}`,
          "info",
        );
      } else {
  ...
  const handleCreateSession = useCallback(
    async (formData: NewSessionFormData) => {
  ...
  // 处理B+树操作接口就绪
  const handleOperationsReady = useCallback(
    (operations: BPlusTreeOperations) => {
  ...
  useEffect(() => {
    ...
    searchParams,
    selectionState.selectedBplusId,
    historyStorage,
    handleSessionSelect,
    setSelectedBplusId,
  ]);
  ...
  // 操作面板设置变更处理
  const handleOperationSettingsChange = useCallback(
    (newSettings: typeof operationSettings) => {
  ...
  const handleToggleHistory = useCallback(() => {
    ...
    showMessage(
      !showHistory ? "已显示历史记录区域" : "已隐藏历史记录区域",
      "info",
    );
              ...
              >
                ...
                {currentSession ? (
                  <BPlusTreeVisualizer
                    ...
                    key={
                      selectedSessionId
                        ? `${selectedSessionId}-${selectedStepIndex}`
                        : "initial-session"
                    }
                    order={order}
