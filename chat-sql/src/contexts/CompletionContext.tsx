...
export const useCompletionContext = () => {
  ...
  if (!context) {
    throw new Error(
      "useCompletionContext must be used within CompletionProvider",
    );
  }
  return context;
};
...
export const CompletionProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const loadCompletionState = useCallback(async (recordId: number) => {
    try {
      const completedProblemsArray =
        await ProgressService.getCompletedProblems(recordId);
      ...
      try {
        ...
        if (result.isNewCompletion) {
          ...
          if (statusInfo.status === "COMPLETED") {
            ...
            messageApi.success(
              `问题 ${problemIndex + 1} 完成！进度: ${result.record.progress}/${result.record.totalProblems}`,
            );
          }
  ...
  const checkQueryResult = useCallback(() => {
    ...
    llmResult.data.outputs.expected_result.forEach(
      (expected: TableTuple, index: number) => {
